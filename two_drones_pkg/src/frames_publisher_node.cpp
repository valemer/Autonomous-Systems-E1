...
class FramesPublisherNode{
 ...
 public:
  ...
  void onPublish(const ros::TimerEvent&){
    ...
    const auto dur = (ros::Time::now() - startup_time);
    double time = dur.sec + dur.nsec * 1e-9;
    ROS_ERROR("%f", time);
    ...
    AV1World.setOrigin({cos(time), sin(time), 0.0});
    tf::Quaternion q;
    q.setRPY(0, 0, time);
    AV1World.setRotation(q);

    AV2World.setOrigin({sin(time), 0.0, cos(2*time)});
